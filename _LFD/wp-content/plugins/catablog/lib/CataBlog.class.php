...
class CataBlog {
	...
	*****************************************************/
	public function registerWordPressHooks() {
		...
		$catablog_widget_callback = function () {
			return register_widget("CataBlogWidget");
		};
		$catablog_category_widget_callback = function () {
			return register_widget("CataBlogCategoryWidget");
		};
		add_action( 'widgets_init', $catablog_widget_callback );
		add_action( 'widgets_init', $catablog_category_widget_callback );
	...
	*****************************************************/
	...
	public function admin_enqueue_scripts() {
		...
		if ($catablog_upload_page) {
			// @TODO: refactor upload javascript into its own js file, like catablog-handlers used to be
	...
	public function frontend_shortcode_catablog($atts) {
		...
		if (isset($this->results_cache)) {
			...
			if (!empty($category)) {
				...
				$walk_func = function (&$val) {
					$val = trim($val);
				};
				array_walk($categories, $walk_func);
...
//			$success_message = '<li class="updated">' . __('Update:', 'catablog') . sprintf(__(' %s updated in database.', 'catablog'), '<em>'.$row['title'].'</em>') . '</li>';
	...
	*****************************************************/
	...
	private function string2slug($string) {
		...
		try {
			$tax = get_taxonomy($this->custom_tax_name);
			$slug = @wp_unique_term_slug($slug, $tax);
		} catch (Exception $e) {
